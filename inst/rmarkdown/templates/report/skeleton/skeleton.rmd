...
## Beta Diversity (Unconstrained Ordinations) {.tabset}     

Here, commonly used metrics for betadiversity are depicted. Additionally, adonis test and Permutation test for homogeneity of multivariate dispersions (betadisper) results where applicable are given for the main variable i.e. stored in VariableA. The default transformation is log10(1 + x) as zeros are common to microbiome count data.     
...
if (UnConstOrd == TRUE){
  ...
  bc.pcoa.plot <- bc.pcoa.plot + theme_bw() + geom_point(size = 3)
  
  print(bc.pcoa.plot)
  
  ggsave(
    "./BetaDiversity/Bray-Curtis distance PCoA.pdf",
    plot = bc.pcoa.plot,
    height = 6,
    width = 10
    )
    ...
    # Adonis test
    print(adonis(ps3_bray ~ VariableA, data = select.meta))
    
    
    # Homogeneity of dispersion test
    beta.bray <- betadisper(ps3_bray, select.meta$VariableA)
    print(permutest(beta.bray))
...
if (UnConstOrd == TRUE){
  ...
  wunifrac.pcoa <-
    ordinate(ps3.log, method = "PCoA", distance = "wunifrac")
    
    wunifrac.pcoa.plot <- plot_ordination(
    ps3.log,
    wunifrac.pcoa,
    type = "split",
    axes = 1:2,
    ...
    wunifrac.pcoa.plot <-
    wunifrac.pcoa.plot + theme_bw() + geom_point(size = 3)
    print(wunifrac.pcoa.plot)
    
    ggsave(
    "./BetaDiversity/Weighted Unifrac distance PCoA.pdf",
    plot = wunifrac.pcoa.plot,
    height = 6,
    width = 10
    )
    ...
    # Adonis test
    print(adonis(ps3_wunifrac ~ VariableA, data = select.meta))
    
    
    # Homogeneity of dispersion test
    beta.wunifrac <- betadisper(ps3_wunifrac, select.meta$VariableA)
    print(permutest(beta.wunifrac))
...
if (UnConstOrd == TRUE){
  ...
  unifrac.pcoa <-
    ordinate(ps3.log, method = "PCoA", distance = "unifrac")
    ...
    unifrac.pcoa.plot <-
    unifrac.pcoa.plot + theme_bw() + geom_point(size = 3)
    print(unifrac.pcoa.plot)
    
    ggsave(
    "./BetaDiversity/Unweighted Unifrac distance PCoA.pdf",
    plot = unifrac.pcoa.plot,
    height = 6,
    width = 10
    )
